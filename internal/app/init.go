...
func branchExistsLocallyOrRemotely(verbose bool, gitPath, dir, branch string) (bool, error) {
	// Check local
	...
	// Check remote
	out, err := RunGit(verbose, dir, gitPath, "ls-remote", "--heads", "origin", branch)
	if err != nil {
		return false, err
	}
	if len(out) > 0 {
...
func validateEnvironment(verbose bool, repos []Repository, gitPath string) error {
	for _, repo := range repos {
		...
		if _, err := os.Stat(gitDir); err == nil {
			...
			if repo.Revision != nil && *repo.Revision != "" && repo.Branch != nil && *repo.Branch != "" {
				exists, err := branchExistsLocallyOrRemotely(verbose, gitPath, targetDir, *repo.Branch)
				if err != nil {
...
func PerformInit(verbose bool, repos []Repository, gitPath string, parallel, depth int) error {
	if err := validateEnvironment(verbose, repos, gitPath); err != nil {
	...
	for _, repo := range repos {
		...
		go func(repo Repository) {
			...
			if shouldClone {
				...
				if err := RunGitInteractive(verbose, "", gitPath, gitArgs...); err != nil {
			...
			if repo.Revision != nil && *repo.Revision != "" {
				...
				if err := RunGitInteractive(verbose, targetDir, gitPath, "checkout", *repo.Revision); err != nil {
				...
				if repo.Branch != nil && *repo.Branch != "" {
					...
					if err := RunGitInteractive(verbose, targetDir, gitPath, "checkout", "-b", *repo.Branch); err != nil {
				...
				if err := RunGitInteractive(verbose, targetDir, gitPath, "checkout", *repo.Branch); err != nil {
...
func handleInit(args []string, opts GlobalOptions) {
	...
	if err := PerformInit(verbose, *config.Repositories, opts.GitPath, parallel, depth); err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
}
