...
func ValidateRepositoriesIntegrity(verbose bool, config *Config, gitPath string) error {
	for _, repo := range *config.Repositories {
		...
		// Check remote origin
		currentURL, err := RunGit(verbose, targetDir, gitPath, "config", "--get", "remote.origin.url")
		if err != nil {
...
func CollectStatus(verbose bool, config *Config, parallel int, gitPath string) []StatusRow {
	var rows []StatusRow
	var mu sync.Mutex
	...
	for _, repo := range *config.Repositories {
		...
		go func(repo Repository) {
			...
			row := getRepoStatus(verbose, repo, gitPath)
			if row != nil {
				mu.Lock()
...
func getRepoStatus(verbose bool, repo Repository, gitPath string) *StatusRow {
	targetDir := GetRepoDir(repo)
	repoName := targetDir
	...
	branchName, err := RunGit(verbose, targetDir, gitPath, "rev-parse", "--abbrev-ref", "HEAD")
	if err != nil {
		branchName = ""
	}
	isDetached := false
	...
	// 2. Get Short SHA
	shortSHA, err := RunGit(verbose, targetDir, gitPath, "rev-parse", "--short", "HEAD")
	if err != nil {
		shortSHA = ""
	}
	...
	localHeadFull, err := RunGit(verbose, targetDir, gitPath, "rev-parse", "HEAD")
	if err != nil {
		localHeadFull = ""
	}
	...
	if !isDetached {
		...
		output, err := RunGit(verbose, targetDir, gitPath, "ls-remote", "origin", "refs/heads/"+branchName)
		if err == nil && output != "" {
			...
			if len(fields) > 0 {
				...
				if localHeadFull != "" {
					count, err := RunGit(verbose, targetDir, gitPath, "rev-list", "--count", localHeadFull+".."+remoteHeadFull)
					if err == nil && count != "0" {
	...
	if remoteHeadFull != "" && localHeadFull != "" {
		...
		_, err := RunGit(verbose, targetDir, gitPath, "cat-file", "-e", remoteHeadFull)
		objectMissing := (err != nil)

		if objectMissing {
			// Fetch to ensure we can calculate status accurately
			_, err := RunGit(verbose, targetDir, gitPath, "fetch", "origin", branchName)
			if err == nil {
		...
		if remoteHeadFull != localHeadFull {
			...
			count, err := RunGit(verbose, targetDir, gitPath, "rev-list", "--count", remoteHeadFull+".."+localHeadFull)
			if err == nil && count != "0" {
				hasUnpushed = true
			}
		}
		...
		if repo.Branch != nil && *repo.Branch != "" && *repo.Branch == branchName {
			if remoteHeadFull != localHeadFull {
				...
				if isPullable && !objectMissing {
					// Check for conflicts
					// 2. Merge Base
					base, err := RunGit(verbose, targetDir, gitPath, "merge-base", localHeadFull, remoteHeadFull)
					if err == nil && base != "" {
						base = strings.TrimSpace(base)
						// 3. Merge Tree
						output, err := RunGit(verbose, targetDir, gitPath, "merge-tree", base, localHeadFull, remoteHeadFull)
						if err == nil {
