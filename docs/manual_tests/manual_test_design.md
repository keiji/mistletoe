...
本書は、`mstl` コマンドラインツールの主要機能を検証するためのシェルスクリプトの設計概要を記述します。テストスクリプトは、Docker 環境での実行を標準として想定しています。

## 1. 概要 (Overview)

テストスクリプトは以下の処理を実行します。

1.  一時的なテスト用ディレクトリを作成します。
2.  サーバー環境をシミュレートするための「リモート」ベア Git リポジトリをセットアップします。
3.  これらのリモートリポジトリを指す `mstl` 設定ファイルを生成します。
4.  `mstl` の各サブコマンドを論理的な順序で実行します。
5.  各コマンドの結果（終了コード、ファイルの存在、Git の状態）を検証します。

## 2. 前提条件 (Prerequisites)

*   Docker (推奨: ビルドおよびテスト実行環境として使用)
    *   コンテナ内に Go, Git, Python, Bash が含まれます。
...
*   **ユーザー確認:** 手動テストの開始時に、テスト手順と変更内容（作成されるリポジトリなど）をユーザーに提示し、続行する前に確認を求めます。
*   **テーブル出力:** テーブルは Markdown 形式で出力します。
