...
*   **リポジトリ構成**: 4つのリポジトリ（A, B, C, D）を使用します。
*   **依存関係**:
...
1.  **PRの作成**: リポジトリ A, B, C, D すべてに対して Pull Request が作成されていること。
2.  **依存関係の表示**:
    ...
    *   **リポジトリ D の PR**: 他のリポジトリへのリンクが含まれていないこと（Others セクションに含まれる場合があるが、Dependencies/Dependents は空であること）。
