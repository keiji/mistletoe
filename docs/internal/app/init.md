...
*   **存在確認**:
    *   **存在する場合**: パスがディレクトリであることを確認。さらに、ディレクトリが**空である**ことを確認します。ファイルである場合や、空でないディレクトリの場合はエラーとなります。
...
2.  **ブランチ競合チェック**
    ...
    *   検証では、指定された `branch` が既に存在するか（ローカルまたはリモート）を確認。存在する場合、既存のブランチの上書きを防ぐために検証失敗
