...
| `--dependencies` | | 依存関係グラフ（Mermaid形式）を含むMarkdownファイルのパス | なし |
| `--depth` | | 指定されたコミット数に履歴を切り詰めてシャロークローンを作成 | 0 (フルクローン) |
| `--jobs` | `-j` | クローン/チェックアウトに使用する並列プロセス数 | 1 |
| `--yes` | `-y` | プロンプト（安全確認や設定ファイルの自動検索など）を自動承諾します。 | false |
...
1.  **適格性チェック (Eligibility Check)**
    ...
    **注意**: `branch` または `base-branch` がローカルに存在しないがリモート（`origin`）には存在する場合、検証プロセス中に自動的にリモートから取得（fetch）されます。これにより、後続のチェックアウト操作などが正常に行われることを保証します。リモートにも存在しない場合はエラーとなります。
...
3.  **依存関係グラフ保存**:
    *   `--dependencies` が指定されている場合: 指定されたファイルを読み込み、`.mstl/dependency-graph.md` として保存します。指定されたファイルの内容が不正（存在しないIDが含まれる等）な場合は、`init`処理の開始前に検証エラーとなります。
    *   `--dependencies` が指定されていない場合: メモリ上のリポジトリ一覧を基に、依存関係のないノードのみの Mermaid グラフを生成し、`.mstl/dependency-graph.md` として保存します。
