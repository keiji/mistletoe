...
1.  **更新の必要性 (Needs Pull)**:
    *   少なくとも1つのリポジトリが `IsPullable`（リモートより遅れている）状態であれば、更新が必要と判断されます。

2.  **戦略の必要性 (Needs Strategy)**:
    *   リポジトリが `IsPullable`（遅れている）かつ `HasUnpushed`（進んでいる）場合、つまりローカルとリモートが分岐（Diverged）している場合、ユーザー入力が必要となります。
    *   この場合、ユーザーに `Merge`, `Rebase`, または `Abort` の選択を求めます。
    *   分岐しているリポジトリがない場合、プロンプトは表示されず、デフォルトの `git pull`（通常は Fast-forward または設定依存）が実行されます。

### 3.3. 実行ロジック (Execution Logic)

実際の `git pull` 操作は、安全のため**順次（シーケンシャル）**に実行されます。

*   **対象**: 設定ファイル内の全リポジトリ。ただし、リモートブランチが存在しない（`RemoteRev` が空）リポジトリはスキップされます。
*   **エラーハンドリング**: いずれかのリポジトリで `git pull` が失敗した場合、プロセスは即座にエラー終了します。これにより、問題のある状態で処理が続行されるのを防ぎます。
